...
// ─── SYMBOLS ─────────────────────────────────────────────────────
const SYMBOLS = [
    { id: 'f1',      emoji: '🏎️',  label: 'Formula 1',  payout3: 100, payout2: 5,  weight: 2 },
    { id: 'super',   emoji: '🚀',  label: 'Rocket',     payout3: 50,  payout2: 3,  weight: 3 },
    { id: 'sports',  emoji: '🚗',  label: 'Sports Car', payout3: 20,  payout2: 2,  weight: 5 },
...
// Generate a reel strip of N symbols
const makeStrip = (n = 30) => Array.from({ length: n }, randomSymbol);

const REEL_COUNT = 3;
const VISIBLE_ROWS = 3;
const SYMBOL_HEIGHT = 90; // px
const PAYLINE_INDEX = 1;   // middle row is the payline

// ─── WIN EVALUATION ──────────────────────────────────────────────
const evaluate = (results, betAmount) => {
    // results = array of 3 symbol objects (the middle/payline row)
    const [a, b, c] = results;
    let winnings = 0;
    let lines = [];
...
const Reel = ({ strip, targetIndex, spinning, delay, onDone }) => {
    ...
    React.useEffect(() => {
        ...
        // Full scroll distance: many full rotations + land on targetIndex
        ...
        const frame = (ts) => {
            ...
            el.scrollTop = startScroll + spinDistance * eased;
    ...
    return (
        <div style={{
            ...
            >
                {/* Duplicate strip for seamless feel */}
...
const Slots = () => {
    ...
    const [results, setResults] = useState(null); // symbols on payline after spin
    ...
    const handleSpin = useCallback(() => {
        ...
        // Pick random landing index for each reel (payline = middle row)
        const newTargets = strips.map(strip =>
            Math.floor(Math.random() * strip.length)
        );
        setTargets(newTargets);

        // Results = symbol at payline for each reel
        const paylineSymbols = newTargets.map((idx, r) => strips[r][idx]);

        // Will be evaluated when all reels stop
        const onAllDone = () => {
        ...
        // Use a timeout fallback to trigger evaluation
        ...
        <>
            ...
            <div className="content-body">
                <div className="content-inner">
                    ...
                    {isJackpot && (
                        <div style={{
                            background: 'linear-gradient(135deg, rgba(255,255,255,0.2), rgba(200,0,17,0.05))',
                            ...
                            <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#E8000F' }}>
                                ...
                                <div className="card-body" style={{ padding: '32px 24px' }}>
                                    ...
                                    <div style={{
                                        ...
                                        <div style={{ textAlign: 'center', marginBottom: 20 }}>
                                            <div style={{ fontSize: '1rem', fontWeight: 700, color: '#E8000F', letterSpacing: '0.15em', textTransform: 'uppercase' }}>
                                                🏎️ &nbsp;MK Car Slots&nbsp; 🏎️
                                            </div>
                                        </div>
                                        ...
                                        <tbody>
                                            {SYMBOLS.map(s => (
                                                <tr key={s.id}>
                                                    ...
                                                    <td>
                                                        <span style={{ color: '#E8000F', fontWeight: 700 }}>
                                                            {s.payout3}x
                                                        </span>
                                                    </td>
                                                    <td>
                                ...
                                <div className="card-body">
                                    ...
                                    <div style={{
                                        ...
                                        <div className="d-flex justify-content-between">
                                            ...
                                            <span style={{ color: '#E8000F', fontWeight: 700 }}>{(bet * 100).toFixed(2)} USDT</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            {showDeposit && (
                <div className="modal-overlay" onClick={() => setShowDeposit(false)}>
                    <div className="modal-box" onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ marginTop: 16, padding: 12, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.2)', borderRadius: 10, fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                            ⚠️ Only send <strong style={{ color: '#E8000F' }}>USDT TRC-20</strong>. Min: <strong>10 USDT</strong>.
                        </div>
